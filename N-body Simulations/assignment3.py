...
def get_orbital_elements_of_triple(stars):
    ...
    return ain, ein, aout, eout, inc_in, outc_out
...
def evolve_triple_with_wind(M1, M2, M3, Pora, Pin_0, ain_0, aout_0,
    ...
    dt_diag = t_end/float(nsteps) # how often we want to save the generated data
    t_diag = dt_diag
    ...
    def advance_stellar_with_massloss(ts, time_framework, dt_massloss=15e-9 | units.Myr, max_massloss=3e-4 | units.MSun):# Scheme 3: dt 8*10^-7, dm = 2*10^-5 (
        ...
        while massloss < max_massloss:
            ...
            delta_t = ctime.time() - t0
            time_framework += delta_t
            massloss = 2.*(mass_0 - numpy.sum(stellar.particles.mass)) # massloss over full gravtiational time step is x2
    ...
    while time < t_end:
        ...
        dt = dtse_fac*Pin
        dt *= random.random() # time step is chosen random between 0 and 50*Pin
        ...
        if scheme == 1:
            ...
            global_massloss = numpy.append(global_massloss, mass_init.value_in(units.MSun) - numpy.sum(stellar.particles.mass).value_in(units.MSun))
            ...
            ts, time_framework, dE_se, dt_stellar = advance_stellar_with_massloss(ts, time_framework)
            ...
            global_time = numpy.append(global_time, time.value_in(units.Myr))
            global_massloss = numpy.append(global_massloss, mass_init.value_in(units.MSun) - numpy.sum(stellar.particles.mass).value_in(units.MSun))
            global_dmdt = numpy.append(global_dmdt, (mass_init.value_in(units.MSun) - numpy.sum(stellar.particles.mass).value_in(units.MSun))/(time.value_in(units.Myr) - time_init.value_in(units.Myr)) )
...
def main(M1, M2, M3, Pora, Pin, ain, aout, ein, eout,
    ...
    i = 0
    for s in srange:
        time, ai, ei, ao, eo,  inci, inco, time_framework = evolve_triple_with_wind(M1, M2, M3,
                                                       Pora, Pin,
                                                       ain, aout,
                                                       ein, eout,
                                                       t_end, nsteps,
    ...
    data = numpy.array([total_time, time_framework, time_framework/total_time * 100])
...
def new_option_parser():
    ...
    result.add_option("-t", unit=units.Myr,
                      dest="t_end", type="float", default = 0.1 | units.Myr,
                      help="end time of the simulation [%default]")
    result.add_option("-s",
                      dest="scheme", type="int", default = 4,
                      help="integration scheme [%default]")
    result.add_option("--dtse",
                      dest="dtse_fac", type="float", default = 50, # time step as a fraction of the orbital period
